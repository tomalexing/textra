  ...
  componentDidMount() {
    ...
    class Emitter {
      ...
      getNoise(x, y, channel) {
        ...
        return (
          this.noiseData[(~~x + ~~y * this.width) * 4 + channel] / 127 - 1.0
        );
      }
      ...
      update() {
        ...
        this.particles.forEach(p => {
          p.vx =
            p.vx * damping +
            this.getNoise(p.x, p.y, 0) * 4 * noise +
            fuzzy(0.1) * fuzz;
          p.vy =
            p.vy * damping +
            this.getNoise(p.x, p.y, 1) * 4 * noise +
            fuzzy(0.1) * fuzz;
          p.age++;
          times(10, x => {
            p.x += p.vx * 0.1;
            ...
            this.data[index] = this.tonemap(
              (this.hdrdata[index] += r * intensity)
            );
            this.data[index + 1] = this.tonemap(
              (this.hdrdata[index + 1] += g * intensity)
            );
            this.data[index + 2] = this.tonemap(
              (this.hdrdata[index + 2] += b * intensity)
            );
          });
          if (p.age < maxAge) {
            alive.push(p);
    ...
    class Smoke {
      ...
      update() {
        ...
        ctx.fillStyle = "rgba(34,143,216, 1.00)";
    ...
    let smoke = new Smoke(document.getElementById("smoke"), {
      width: WIDTH,
      height: HEIGHT
    });
  }
        ...
        <div className="outer">
            <div className="cover">
                <div className="container f f-col f-align-2-2 stuff">
                    <h1 className="h1">TEXTRA</h1>
                    <p>Сервис быстрых переводов</p>
                    <p>Quick tranlator service</p>
                    <div className="f stuff__bottom">
                        <Link className="btn btn-primiry btn-normal " to={{pathname:"/signup", state:{from:'/'}}} >Зарегистрироваться</Link>
